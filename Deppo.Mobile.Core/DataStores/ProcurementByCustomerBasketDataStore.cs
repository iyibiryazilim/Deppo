...
public class ProcurementByCustomerBasketDataStore : IProcurementByCustomerBasketService
{
    ...
    {
        var content = new StringContent(JsonConvert.SerializeObject(ProcurementByCustomerBasketQuery(firmNumber, periodNumber, warehouseNumber,itemsReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
...
private string ProcurementByCustomerBasketQuery(int firmNumber, int periodNumber, int warehouseNumber, int[] itemsReferenceId, string search = "", int skip = 0, int take = 20)
    {
...
LEFT JOIN LG_{firmNumber.ToString().PadLeft(3, '0')}_UNITSETL AS UNITSETL WITH(NOLOCK) ON LGMAIN.UOMREF = UNITSETL.LOGICALREF
