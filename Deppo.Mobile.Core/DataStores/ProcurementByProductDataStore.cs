...
public class ProcurementByProductDataStore : IProcurementByProductService
{
    ...
    {
        var content = new StringContent(JsonConvert.SerializeObject(ProcurementByProductListQuery(firmNumber, periodNumber, warehouseNumber, search, skip, take)), Encoding.UTF8, "application/json");
    ...
    private string ProcurementByProductListQuery(int firmNumber, int periodNumber, int warehouseNumber, string search = "", int skip = 0, int take = 20)
    {
...
WHERE ITEMS.LOGICALREF IS NOT NULL AND ORFLINE.CLOSED = 0 AND ORFLINE.USREF <> 0 AND (ORFLINE.AMOUNT - ORFLINE.SHIPPEDAMOUNT) > 0 AND ORFLINE.TRCODE = 1 AND ORFLINE.SOURCEINDEX = {warehouseNumber}";
