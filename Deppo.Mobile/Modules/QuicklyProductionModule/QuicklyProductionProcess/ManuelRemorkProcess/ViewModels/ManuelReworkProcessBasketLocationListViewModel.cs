	...
	private async Task PerformSearchAsync(Entry barcodeEntry)
	{
		...
		try
		{
			...
			if (result.IsSuccess)
			{
				...
				{
					_userDialogs.ShowToast($"{barcodeEntry.Text} kodlu raf bulunamadı.");
					return;
				}
				...
				foreach (var item in result.Data)
				{
					...
					if(SelectedItems.Any(x => x.Code == obj.Code))
					{
						_userDialogs.ShowToast($"{barcodeEntry.Text} kodlu raf zaten bulunmakta.");
