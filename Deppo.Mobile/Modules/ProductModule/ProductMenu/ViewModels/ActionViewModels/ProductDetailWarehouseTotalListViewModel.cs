...
namespace Deppo.Mobile.Modules.ProductModule.ProductMenu.ViewModels.ActionViewModels
{
    ...
    public partial class ProductDetailWarehouseTotalListViewModel : BaseViewModel
    {
        private readonly IUserDialogs _userDialogs;
        private readonly IHttpClientService _httpClientService;
        private readonly IProductDetailActionService _productDetailActionService;

        public ProductDetailWarehouseTotalListViewModel(IUserDialogs userDialogs, IHttpClientService httpClientService, IProductDetailActionService productDetailActionService)
        {
        ...
        public async Task LoadItemsAsync()
        {
            ...
            try
            {
                ...
                var result = await _productDetailActionService.GetWarehouses(httpClient, _httpClientService.FirmNumber, _httpClientService.PeriodNumber, ProductDetailModel.Product.ReferenceId, string.Empty, 0, 20);
                if (result.IsSuccess)
                {
                    if (result.Data == null)
                        return;
        ...
        public async Task LoadMoreItemsAsync()
        {
            ...
            try
            {
                IsBusy = true;
                var httpClient = _httpClientService.GetOrCreateHttpClient();
                var result = await _productDetailActionService.GetWarehouses(httpClient, _httpClientService.FirmNumber, _httpClientService.PeriodNumber, ProductDetailModel.Product.ReferenceId, string.Empty, Items.Count, 20);
                if (result.IsSuccess)
                {
                    if (result.Data == null)
                        return;
