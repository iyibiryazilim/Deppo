...
public partial class CustomerDetailViewModel : BaseViewModel
{
    ...
    private async Task GetLastTransactionsAsync(HttpClient httpclient)
    {
		...
		WHERE STLINE.IOCODE IN (1,2,3,4) AND STFICHE.TRCODE IN (1,2,3,7,6,8)  AND  CLCARD.LOGICALREF = {CustomerDetailModel.Customer.ReferenceId}  ORDER BY STLINE.DATE_ DESC";
