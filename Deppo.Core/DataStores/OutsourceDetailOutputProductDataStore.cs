...
{
    public class OutsourceDetailOutputProductDataStore : IOutsourceDetailOutputProductService
    {
        private string postUrl = "/gateway/customQuery/CustomQuery";

        public async Task<DataResult<IEnumerable<dynamic>>> GetObjects(HttpClient httpClient, int firmNumber, int periodNumber, int outsourcereReferenceId, string search = "", int skip = 0, int take = 20)
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetOutputProductByOutsourceQuery(firmNumber, periodNumber, outsourcereReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        private string GetOutputProductByOutsourceQuery(int firmNumber, int periodNumber, int outsourcereReferenceId, string search, int skip, int take)
        {
            string baseQuery = $@"
    SELECT * FROM (
        SELECT
            [ReferenceUd] = ITEMS.LOGICALREF,
