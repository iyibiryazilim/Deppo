...
namespace Deppo.Core.DataStores
{
    public class ProductCountingDataStore : IProductCountingService
    {
        ...
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetWarehouses(firmNumber, periodNumber, productReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        public async Task<DataResult<IEnumerable<dynamic>>> GetWarehousesByVariant(HttpClient httpClient, int firmNumber, int periodNumber, int variantReferenceId, string search = "", int skip = 0, int take = 20)
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetWarehousesByVariant(firmNumber, periodNumber, variantReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        private string GetWarehouses(int firmNumber, int periodNumber,int productReferenceId, string search = "", int skip = 0, int take = 20)
        {
...
FETCH NEXT {take} ROWS ONLY;";
        ...
        private string GetWarehousesByVariant(int firmNumber, int periodNumber, int variantReferenceId, string search = "", int skip = 0, int take = 20)
        {
            var baseQuery = @$"SELECT
			[ReferenceId] = LGMAIN.LOGICALREF,
			[Number] = LGMAIN.NR,
