...
FETCH NEXT {take} ROWS ONLY;";
    ...
    [Image] = CAST(FIRMDOC.LDATA AS VARBINARY(MAX)),
...
WHERE LGMAIN.CANCELLED = 0 
  ...
  AND ITEMS.CANCONFIGURE = 0";
            ...
            baseQuery += @$"
GROUP BY ITEMS.LOGICALREF,WHOUSE.NR,ITEMS.LOCTRACKING, ITEMS.CODE, ITEMS.NAME,UNITSETF.LOGICALREF, UNITSETF.CODE, UNITSETF.NAME,UNITSETL.LOGICALREF,UNITSETL.CODE, UNITSETL.NAME,VARIANT.LOGICALREF,VARIANT.CODE,VARIANT.NAME, CAST(FIRMDOC.LDATA AS VARBINARY(MAX))
ORDER BY ITEMS.CODE
OFFSET {skip} ROWS 
FETCH NEXT {take} ROWS ONLY;";
