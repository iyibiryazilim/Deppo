...
{
    public class PurchaseAnalysisDataStore : IPurchaseAnalysisService
    {
        private string postUrl = "/gateway/customQuery/CustomQuery";

        //Termin Tarihi Geçen Müşteri Sayısı
        public async Task<DataResult<dynamic>> DueDatePassedSuppliersCount(HttpClient httpClient, int firmNumber, int periodNumber)
        {
            var content = new StringContent(JsonConvert.SerializeObject(DueDatePassedSuppliersCount(firmNumber, periodNumber)), Encoding.UTF8, "application/json");
        ...
        //SatınAlınan Malzeme Referans Sayısı
        public async Task<DataResult<dynamic>> PurchaseProductReferenceCount(HttpClient httpClient, int firmNumber, int periodNumber)
        {
            var content = new StringContent(JsonConvert.SerializeObject(PurchaseProductReferenceCount(firmNumber, periodNumber)), Encoding.UTF8, "application/json");
        ...
        public async Task<DataResult<IEnumerable<dynamic>>> LastSuppliers(HttpClient httpClient, int firmNumber, int periodNumber)
        {
            var content = new StringContent(JsonConvert.SerializeObject(LastSupplier(firmNumber, periodNumber)), Encoding.UTF8, "application/json");
...
";
        ...
        private string LastSupplier(int firmNumber, int periodNumber)
        {
