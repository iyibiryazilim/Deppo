...
namespace Deppo.Core.DataStores
{
    public class OutsourceDetailAllFichesDataStore : IOutsourceDetailAllFichesService
    {
        ...
        public async Task<DataResult<IEnumerable<dynamic>>> GetAllFiches(HttpClient httpClient, int firmNumber, int periodNumber, int outsourceReferenceId, string search = "", int skip = 0, int take = 20, string externalDb = "")
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetAllFichesByOutsourceQuery(firmNumber, periodNumber, outsourceReferenceId, search, skip, take, externalDb)), Encoding.UTF8, "application/json");
        ...
        private string GetAllFichesByOutsourceQuery(int firmNumber, int periodNumber, int outsourceReferenceId, string search, int skip = 0, int take = 20, string externalDb = "")
        {
