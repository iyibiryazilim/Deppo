...
public class CustomerDetailOutputProductDataStore : ICustomerDetailOutputProductService
{
	...
	{
		var content = new StringContent(JsonConvert.SerializeObject(GetOutputProductByCustomerQuery(firmNumber, periodNumber, customerReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
	...
	private string GetOutputProductByCustomerQuery(int firmNumber, int periodNumber, int customerReferenceId, string search, int skip, int take)
	{
		string baseQuery = $@"
    SELECT * FROM (
        SELECT
