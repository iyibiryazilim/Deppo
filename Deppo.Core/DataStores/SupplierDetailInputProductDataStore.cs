...
public class SupplierDetailInputProductDataStore : ISupplierDetailInputProductService
{
    ...
    {
        var content = new StringContent(JsonConvert.SerializeObject(GetInputProductBySupplierQuery(firmNumber, periodNumber, supplierReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
    ...
    private string GetInputProductBySupplierQuery(int firmNumber, int periodNumber, int supplierReferenceId, string search, int skip, int take)
    {
        string baseQuery = $@"
    SELECT * FROM (
        SELECT
