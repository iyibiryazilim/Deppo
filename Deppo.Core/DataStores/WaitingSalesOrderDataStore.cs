...
public class WaitingSalesOrderDataStore : IWaitingSalesOrderService
{
	...
	public async Task<DataResult<IEnumerable<dynamic>>> GetObjectsByProduct(HttpClient httpClient, int firmNumber, int periodNumber, int warehouseNumber, int customerReferenceId, int productReferenceId, string search = "", int skip = 0, int take = 20)
	{
		var content = new StringContent(JsonConvert.SerializeObject(WaitingSalesOrderQueryByCustomerAndWarehouseAndProduct(firmNumber, periodNumber, warehouseNumber, customerReferenceId, productReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
	...
	private string WaitingSalesOrderQueryByCustomerAndWarehouseAndProduct(int firmNumber, int periodNumber, int warehouseNumber, int customerReferenceId, int productReferenceId, string search = "", int skip = 0, int take = 20)
	{
		string baseQuery = @$"SELECT
			[ReferenceId] = ORFLINE.LOGICALREF,
            [OrderReferenceId] = ORFICHE.LOGICALREF,
        ...
        LEFT JOIN L_CAPIWHOUSE AS WHOUSE ON ORFLINE.SOURCEINDEX = WHOUSE.NR AND WHOUSE.FIRMNR = {firmNumber}
		WHERE ORFLINE.CLOSED = 0 AND (ORFLINE.AMOUNT - ORFLINE.SHIPPEDAMOUNT) > 0 AND ORFLINE.TRCODE = 1
		AND ITEMS.UNITSETREF <> 0 AND CLCARD.LOGICALREF = {customerReferenceId} AND ORFLINE.SOURCEINDEX = {warehouseNumber} AND ORFLINE.STOCKREF = {productReferenceId}
		";
