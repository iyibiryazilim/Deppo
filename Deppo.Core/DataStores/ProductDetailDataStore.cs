...
[OutputQuantity] = ISNULL((SELECT SUM(STLINE.AMOUNT) FROM LG_{firmNumber.ToString().PadLeft(3, '0')}_{periodNumber.ToString().PadLeft(2, '0')}_STLINE AS STLINE WITH(NOLOCK) WHERE TRCODE IN(3,4) AND LINETYPE = 0 AND YEAR(STLINE.DATE_) = {xDate.Year} AND DAY(STLINE.DATE_) = {xDate.Month} AND DAY(STLINE.DATE_) = {xDate.Day} AND STLINE.STOCKREF = {productReferenceId}),0)
UNION All ";
                }
                else
                {
