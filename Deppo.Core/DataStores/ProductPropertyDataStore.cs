...
namespace Deppo.Core.DataStores
{
    public class ProductPropertyDataStore : IProductPropertyService
    {
        ...
        {
            var content = new StringContent(JsonConvert.SerializeObject(ProductPropertyQuery(firmNumber, periodNumber, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        private string ProductPropertyQuery(int firmNumber, int periodNumber, string search = "", int skip = 0, int take = 20)
        {
            string baseQuery = $@"
             SELECT 
