...
{
    public class WarehouseDetailAllFicheListDataStore : IWarehouseDetailAllFicheListService
    {
        private string postUrl = "/gateway/customQuery/CustomQuery";

        public async Task<DataResult<IEnumerable<dynamic>>> GetAllFiches(HttpClient httpClient, int firmNumber, int periodNumber, int warehouseNumber, string search = "", int skip = 0, int take = 20)
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetLastFichesByProductQuery(firmNumber, periodNumber, warehouseNumber, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        public async Task<DataResult<IEnumerable<dynamic>>> GetTransactionsByFiche(HttpClient httpClient, int firmNumber, int periodNumber, int warehouseNumber, int ficheReferenceId, int skip = 0, int take = 20)
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetLastProductTransactionQuery(firmNumber, periodNumber, ficheReferenceId, warehouseNumber)), Encoding.UTF8, "application/json");
        ...
        private string GetLastFichesByProductQuery(int firmNumber, int periodNumber, int warehouseNumber, string search, int skip, int take)
        {
        ...
        private string GetLastProductTransactionQuery(int firmNumber, int periodNumber, int ficheReferenceId, int warehouseNumber)
        {
            string baseQuery = $@"SELECT
		...
		WHERE STFICHE.GRPCODE = 3 AND STFICHE.LOGICALREF = {ficheReferenceId} AND STLINE.SOURCEINDEX={warehouseNumber} AND  STFICHE.PRODSTAT = 0 AND STLINE.LPRODSTAT = 0
