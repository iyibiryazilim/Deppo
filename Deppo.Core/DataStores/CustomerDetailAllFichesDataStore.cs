...
{
    public class CustomerDetailAllFichesDataStore : ICustomerDetailAllFichesService
    {
        private string postUrl = "/gateway/customQuery/CustomQuery";

        public async Task<DataResult<IEnumerable<dynamic>>> GetAllFiches(HttpClient httpClient, int firmNumber, int periodNumber, int customerReferenceId, string search = "", int skip = 0, int take = 20, string externalDb = "")
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetAllFichesByCustomerQuery(firmNumber, periodNumber, customerReferenceId, search, skip, take, externalDb)), Encoding.UTF8, "application/json");
        ...
        private string GetAllFichesByCustomerQuery(int firmNumber, int periodNumber, int customerReferenceId, string search, int skip, int take, string externalDb = "")
        {
		...
		WHERE STFICHE.PRODSTAT = 0 AND STLINE.LPRODSTAT = 0 AND STFICHE.LOGICALREF = {ficheReferenceId}
