...
namespace Deppo.Core.DataStores
{
    public class CustomerDetailAllFichesDataStore : ICustomerDetailAllFichesService
    {
        ...
        public async Task<DataResult<IEnumerable<dynamic>>> GetAllFiches(HttpClient httpClient, int firmNumber, int periodNumber, int customerReferenceId, string search = "", int skip = 0, int take = 20)
        {
            var content = new StringContent(JsonConvert.SerializeObject(GetAllFichesByCustomerQuery(firmNumber, periodNumber, customerReferenceId, search, skip, take)), Encoding.UTF8, "application/json");
        ...
        private string GetAllFichesByCustomerQuery(int firmNumber, int periodNumber, int customerReferenceId, string search, int skip, int take)
        {
			...
			WHERE STFICHE.TRCODE IN (13,12,11,25,51,50) AND STFICHE.PRODSTAT = 0 AND CLCARD.LOGICALREF={customerReferenceId}
