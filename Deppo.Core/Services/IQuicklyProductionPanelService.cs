...
public interface IQuicklyProductionPanelService
{
    ...
    public Task<DataResult<IEnumerable<dynamic>>> GetAllProductionFiches(HttpClient httpClient, int firmNumber, int periodNumber, string search = "", int skip = 0, int take = 20);

    Task<DataResult<IEnumerable<dynamic>>> GetProductionTransactions(HttpClient httpClient, int firmNumber, int periodNumber, int ficheReferenceId, string search = "", int skip = 0, int take = 20);
    ...
    public Task<DataResult<IEnumerable<dynamic>>> GetOutputTransactions(HttpClient httpClient, int firmNumber, int periodNumber, int productReferenceId);
}